.PHONY: check design-check pages-check go-test helm-check build-poc image-poc import-poc deploy-poc e2e-poc
...
check: design-check pages-check go-test helm-check
...
go-test:
	GOCACHE=$(CURDIR)/build/go-cache go test ./...
...
deploy-poc: import-poc
	...
	kubectl -n $(MALZONE_NAMESPACE) rollout restart deployment/malzone-api deployment/malzone-operator deployment/malzone-siem-adapter deployment/malzone-siem-sink
	kubectl -n $(MALZONE_NAMESPACE) rollout status deployment/malzone-api deployment/malzone-operator deployment/malzone-siem-adapter deployment/malzone-siem-sink --timeout=120s
